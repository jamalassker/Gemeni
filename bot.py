...
# ================= CONFIG =================
SYMBOLS = ["BTC/USDT", "ETH/USDT"]  # Reduced for Railway resources
TIMEFRAME = "1m"
CANDLES = 300  # Reduced for memory

INITIAL_BALANCE = 1000.0
RISK_PER_TRADE = 0.01
MAX_OPEN_TRADES = 3  # Reduced for Railway
...
LSTM_MODEL_PATH = os.path.join(MODEL_DIR, "lstm_predictor.h5")  # Changed to .h5 for Keras
...
class ModelManager:
    ...
    async def train_models_async(self, exchange):
        """Train models asynchronously"""
        logger.info("Starting model training...")
        try:
            # This would run in background
            await asyncio.to_thread(self.train_models, exchange)
...
# ================= ORDERBOOK =================
async def orderbook_imbalance(symbol, exchange):
    """Calculate order book imbalance"""
    try:
        ob = await exchange.watch_order_book(symbol)
        bid = sum(b[1] for b in ob["bids"][:5])  # Reduced for speed
...
def ai_probability(r, bias):
    ...
    try:
        ...
        ema_diff = abs(r["ema20"] - r["ema50"]) if not pd.isna(r["ema20"]) and not pd.isna(r["ema50"]) else 0
...
class TradingBot:
    ...
    async def initialize(self):
        ...
        tg("🤖 AI 1m Scalper v3 Started on Railway")
        ...
        if model_manager.should_retrain():
            ...
            await model_manager.train_models_async(self.exchange)
    ...
    async def optimize_params(self):
        ...
        for s in SYMBOLS:
            try:
                ohlc = await self.exchange.fetch_ohlcv(s, TIMEFRAME, limit=200)
    ...
    async def manage_trades(self, symbol, r):
        """Manage open trades"""
        trades_to_remove = []
    ...
    async def check_entry(self, symbol, df, r, imbalance, combined_prob):
        ...
        if (
            ...
            logger.info(f"Entered trade for {symbol}")
            return True
    ...
    async def run_iteration(self):
        ...
        try:
            ...
            for s in SYMBOLS:
                try:
                    # Get data
                    candles = await self.exchange.fetch_ohlcv(s, TIMEFRAME, limit=CANDLES)
                    if len(candles) < 50:
                        continue
                    ...
                    # Get predictions
                    imbalance = await orderbook_imbalance(s, self.exchange)
                    ...
                    await self.manage_trades(s, r)
                    
                    # Check for new entry
                    await self.check_entry(s, df, r, imbalance, combined_prob)
        ...
        while self.running:
            try:
                if in_session():
                    ...
                    # Outside session, check less frequently
...
async def main():
    """Main entry point"""
    logger.info("=" * 50)
    logger.info("AI 1M SCALPER BOT STARTING")
    logger.info("=" * 50)
